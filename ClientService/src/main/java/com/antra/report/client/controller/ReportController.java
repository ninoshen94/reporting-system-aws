...
public class ReportController {
    ...
    public ResponseEntity<GeneralResponse> modifyReport(@PathVariable String reqId,@PathVariable String method, @RequestBody @Validated ReportRequest request) {
        log.info("Got Request to modify report with id: {} - {}: {}", reqId, method, request);
        request.setReqId(reqId);
        return method.equals("sync") ? ResponseEntity.ok(new GeneralResponse(reportService.modifySyncReport(request))) : ResponseEntity.ok(new GeneralResponse(reportService.modifyAsyncReport(request)));
    }

    @GetMapping("/report/info/{reqId}")
    public ResponseEntity<GeneralResponse> queryInfo(@PathVariable String reqId) {
        log.info("Got Request to Query Entity by Id: {}", reqId);
        return ResponseEntity.ok(new GeneralResponse(reportService.getInfo(reqId)));
    }
